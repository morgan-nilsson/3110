...
function initTextures(gl, initial_points, depth, a_Position, u_TransformMatrix, rotation_radians) {
    ...
    image.onload = function() {
        ...
        drawSTriangle(initial_points, depth, gl, a_Position, u_TransformMatrix, rotation_radians, u_Sampler);
    };
...
function drawSTriangle(my_points, level, gl, a_Position, u_TransformMatrix, rotation_radians, u_Sampler) {
    ...
    if (level == 0) {
        ...
        const coords = [
            my_points[0], my_points[1], 0.5, 1.0,
            my_points[2], my_points[3], 0.0, 0.0,
            my_points[4], my_points[5], 1.0, 0.0
        ];

        const n = initVertexBuffers(gl, a_Position, coords, 3);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(coords), gl.STATIC_DRAW);

        const textureIndex = triangle_texture_cache[0];
        triangle_texture_cache.shift();
        triangle_texture_cache.push(textureIndex);

        console.log("Using texture index: " + textureIndex);
...
function main() {
    ...
    document.addEventListener("keydown", (event) => {
        ...
        triangle_texture_cache.length = 0;
        if (triangle_texture_cache.length === 0) {
